      ...
      <main className="min-h-screen bg-[#f9f9f9] pt-28 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <div className="h-1 bg-[#e63946] w-16 mx-auto mt-6 rounded" />
          </div>
            ...
            {blogData.map((post) => (
              <article key={post.slug} className="bg-white rounded-2xl shadow-md border border-[#e5e5e5] overflow-hidden flex flex-col group hover:shadow-xl transition-shadow duration-300">
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                    <time className="text-xs font-bold text-[#e63946] tracking-wider uppercase mb-2 block">
                      {new Date(post.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                    </time>
                    <h2 className="font-bold text-xl text-[#000000] font-display mb-3 group-hover:text-[#e63946] transition-colors line-clamp-2">
                  ...
                  <Link 
                    href={`/blog/${post.slug}`}
                    className="inline-flex items-center text-sm font-bold text-[#e63946] hover:text-[#dc2626] transition-colors"
                  >
                    Read Article →
                  </Link>
                </div>
